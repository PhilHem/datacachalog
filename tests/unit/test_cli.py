...
class TestCatalogClean:
    ...
    def test_clean_with_orphaned_files(
        ...
        # Add multiple orphaned cache entries
        for i in range(3):
            orphaned_file = cache_dir / f"orphaned{i}.csv"
            orphaned_file.write_text(f"orphaned{i}")
            orphaned_meta = cache_dir / f"orphaned{i}.csv.meta.json"
            orphaned_meta.write_text(
    ...
    def test_clean_preserves_valid_cache(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """clean preserves valid dataset cache entries."""
        # Create source files
        storage_dir = tmp_path / "storage"
        storage_dir.mkdir()
        source_file1 = storage_dir / "data1.csv"
    ...
    def test_clean_no_catalogs_found_exits_with_error(
        ...
        assert "catalog init" in result.output.lower()

    def test_clean_catalog_load_error_shows_graceful_error(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """clean shows graceful error when catalog file has syntax errors."""
        catalogs_dir = tmp_path / ".datacachalog" / "catalogs"
        catalogs_dir.mkdir(parents=True)
    ...
    def test_clean_preserves_glob_dataset_cache_keys(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """clean preserves glob dataset cache keys (format: dataset_name/path/to/file.ext)."""
        # Create source files for glob pattern
        storage_dir = tmp_path / "storage"
        storage_dir.mkdir()
        ...
        (catalogs_dir / "default.py").write_text(
            ...
            from datacachalog import Dataset
            datasets = [
                Dataset(name="monthly_data", source="{storage_dir}/*.parquet"),
            ]
        """)
        )
    ...
    def test_clean_removes_orphaned_glob_prefix_keys(
        ...
        # Create catalog with regular (non-glob) dataset
        catalogs_dir = tmp_path / ".datacachalog" / "catalogs"
        catalogs_dir.mkdir(parents=True)
        (catalogs_dir / "default.py").write_text(
            dedent(f"""\
        ...
        # Add orphaned key that looks like glob key but doesn't match any dataset
        orphaned_file = cache_dir / "nonexistent_dataset" / "file.csv"
        orphaned_file.parent.mkdir()
        orphaned_file.write_text("orphaned")
    ...
    def test_clean_removes_orphaned_versioned_looking_keys(
        ...
        # Add orphaned key that matches versioned pattern but is orphaned
        # (Note: The clean_orphaned logic preserves ALL versioned-looking keys,
        # so this test verifies that orphaned versioned keys are actually preserved,
        # not removed. This is the current behavior - versioned keys are always preserved.)
        orphaned_versioned_key = "2024-01-15T120000.orphaned"
        orphaned_file = cache_dir / orphaned_versioned_key
        orphaned_file.write_text("orphaned versioned")
        orphaned_meta = cache_dir / f"{orphaned_versioned_key}.meta.json"
        orphaned_meta.write_text(
            '{"etag": "orphaned", "cached_at": "2024-01-01T00:00:00", "source": ""}'
        )

        # Add another orphaned key that doesn't match versioned pattern
        orphaned_file2 = cache_dir / "orphaned.txt"
        orphaned_file2.write_text("orphaned")
        orphaned_meta2 = cache_dir / "orphaned.txt.meta.json"
    ...
    def test_clean_with_empty_cache_directory(
        ...
        cache_dir = tmp_path / "data"
        cache_dir.mkdir()  # Empty cache directory
        monkeypatch.chdir(tmp_path)
    ...
    def test_clean_with_mixed_valid_glob_and_orphaned(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """clean removes only orphaned keys when cache contains both valid glob dataset keys and orphaned keys."""
        # Create source files for glob pattern
        storage_dir = tmp_path / "storage"
        storage_dir.mkdir()
        (storage_dir / "2024-01.parquet").write_text("jan")
        ...
        orphaned_keys = ["orphaned1.txt", "orphaned2.csv"]
        for key in orphaned_keys:
            orphaned_file = cache_dir / key
            orphaned_file.write_text(f"orphaned {key}")
