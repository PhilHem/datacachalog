...
class TestListStatusIntegration:
    ...
    def test_list_status_integration_multiple_catalogs(
        ...
        # Both should show fresh status (check for "fresh" twice in Status column)
    ...
    def test_list_status_integration_catalog_filter(
        ...
        assert "fresh" in result2.output  # Status column shows "fresh" (not "[fresh]")
        # Should NOT show customers from core catalog
        assert "customers" not in result2.output or "core" not in result2.output

    def test_list_table_integration_basic(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Verify table renders correctly with filesystem adapter."""
        storage_dir = tmp_path / "storage"
        storage_dir.mkdir()
        ...
        (catalogs_dir / "default.py").write_text(
            ...
            datasets = [
                ...
                Dataset(name="orders", source="{source_file}"),
            ]
        """)
        )
        ...
        # Rich table may render differently based on terminal capabilities
        # Check for either table format (with borders) or structured output
        has_table_format = "│" in result.output or "┃" in result.output
        has_headers = "Name" in result.output and "Source" in result.output
        # In some environments Rich may output differently, so verify core functionality
        assert has_table_format or has_headers, (
            "Output should have table format or headers"
        )

    def test_list_table_integration_with_status_flag(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Verify table with status column works end-to-end."""
        storage_dir = tmp_path / "storage"
        storage_dir.mkdir()
        ...
        assert result.exit_code == 0
        # Verify table structure with Status column
        assert "│" in result.output or "┃" in result.output  # Table borders
        assert "Status" in result.output  # Status column header
    ...
    def test_list_table_integration_multiple_catalogs(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Verify table formatting with multiple catalogs."""
        storage_dir = tmp_path / "storage"
        storage_dir.mkdir()
        ...
        assert "│" in result.output or "┃" in result.output  # Table borders
        # Verify catalog prefixes in Name column
        assert "core/customers" in result.output
        assert "analytics/metrics" in result.output
        # Verify proper table structure with columns
